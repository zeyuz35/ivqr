### Meta -------------------------
###
### Title: Compute IQR estimator by preprocessing MILP using HiGHS
###
...
### outliers to speed up the procedure using the HiGHS solver.
###
### Author: Omkar A. Katta (adapted for HiGHS)
###

### preprocess_iqr_milp_highs -------------------------
#' Compute IQR estimator by preprocessing MILP using HiGHS
#'
#' @inheritParams preprocess_iqr_milp
#' @param ... Arguments passed to \code{iqr_milp_highs}
#'
#' @export
preprocess_iqr_milp_highs <- function(
  Y,
  D,
  X,
  Z,
