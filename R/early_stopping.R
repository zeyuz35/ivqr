...
iqr_milp_es <- function(
  beta_D_index, # which coefficient are we interested in?
  ...
  vtype <- c(
    ...
    rep("C", n)
  ) # l # NOTE: modificaiton from iqr_milp
  ...
  iqr$obj <- obj
  iqr$A <- rbind(
    A_es, # NOTE: modification from iqr_milp
    A_fix, # `fix`
  ...
  iqr$rhs <- c(
    b_es, # NOTE: modification from iqr_milp
    b_fix, # `fix`
  ...
  iqr$sense <- c(
    sense_es, # NOTE: modification from iqr_milp
    sense_fix, # `fix`
...
iqr_milp_es_gp <- function(
  ...
  if (is.null(O) == F) {
    O <- sort(O)
  } #put index of pos and neg outlier together
  I = setdiff(1:n, O)
  ...
  beta_D_obj[beta_index] <- 1 # new objective
  obj_core = c(
    rep(0, 2 * p_Z),
    beta_D_obj,
    rep(0, p_X),
  ...
  A_pf = cbind(
    ...
    D,
    X,
    diag(as.numeric(is.element(1:n, Iu)))[, is.element(1:n, Iu)],
    -diag(as.numeric(is.element(1:n, Iv)))[, is.element(1:n, Iv)],
    matrix(0, n, n_I + n_I + n_I)
  )
  b_pf = Y
  sense_pf = rep("=", n)
  ...
  #dual feasibility
  A_df_X = cbind(
    matrix(0, p_X, 2 * p_Z + p_D + p_X),
    matrix(0, p_X, n_Iu + n_Iv),
    t(X[I, ]),
    matrix(0, p_X, n_I + n_I)
  )
  if (is.null(O)) {
    b_df_X = (1 - tau) * t(X) %*% ones
  }
  if (is.null(O) == F) {
    b_df_X = (1 - tau) * t(X) %*% ones - t(X[O, ]) %*% a_O
  }
  sense_df_X = rep("=", p_X)

  A_df_Z = cbind(
    matrix(0, p_Z, 2 * p_Z + p_D + p_X),
    matrix(0, p_Z, n_Iu + n_Iv),
    t(Z[I, ]),
    matrix(0, p_Z, n_I + n_I)
  )
  if (is.null(O)) {
    b_df_Z = (1 - tau) * t(Z) %*% ones
  }
  if (is.null(O) == F) {
    b_df_Z = (1 - tau) * t(Z) %*% ones - t(Z[O, ]) %*% a_O
  }
  sense_df_Z = rep("=", p_Z)
  ...
  A_vl_I = cbind(
    matrix(0, n_I, (2 * p_Z + p_D + p_X)),
    matrix(0, n_I, n_Iu),
    diag(as.numeric(is.element(Iv, I)))[is.element(Iv, I), ],
    matrix(0, n_I, n_I),
    matrix(0, n_I, n_I),
    -diag(M_l[I]) * diag(n_I)
  ) #v<=lM # Each v,l has a unique M_l (TODO: remove diag(n_I))
  b_vl_I = rep(0 + eps, n_I)
  sense_vl_I = rep("<=", n_I)

  A_uk_I = cbind(
    matrix(0, n_I, (2 * p_Z + p_D + p_X)),
    diag(as.numeric(is.element(Iu, I)))[is.element(Iu, I), ],
    matrix(0, n_I, n_Iv),
    matrix(0, n_I, n_I),
    -diag(M_k[I]) * diag(n_I),
    matrix(0, n_I, n_I)
  ) #u<=kM # Each u,k has a unique M_l (TODO: remove diag(n_I))
  b_uk_I = rep(0 + eps, n_I)
  sense_uk_I = rep("<=", n_I)

  A_ak_I = cbind(
    matrix(0, n_I, (2 * p_Z + p_D + p_X + n_Iu + n_Iv)),
    diag(n_I),
    -diag(n_I),
    matrix(0, n_I, n_I)
  ) #a_i >= k_i
  ...
  if (out$status != "TIME_LIMIT") {
    ...
    v_Iv = x[
      (2 * p_Z + p_D + p_X + n_Iu + 1):(2 * p_Z + p_D + p_X + n_Iu + n_Iv)
    ]
    a_I = x[
      (2 * p_Z + p_D + p_X + n_Iu + n_Iv + 1):(2 *
        p_Z +
        p_D +
        p_X +
        n_Iu +
    ...
    k_I = x[
      (2 * p_Z + p_D + p_X + n_Iu + n_Iv + n_I + 1):(2 *
        p_Z +
        p_D +
        p_X +
        n_Iu +
    ...
    l_I = x[
      (2 * p_Z + p_D + p_X + n_Iu + n_Iv + 2 * n_I + 1):(2 *
        p_Z +
        p_D +
        p_X +
        n_Iu +
